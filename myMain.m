...
if isillumination==1 && (i==16 || i==17)
    temp=temp*illumination_factor;
end
subplot(5 , 4 , i);
imshow(temp);
...
for i=1:numel(myindexes)
    ...
    imshow(temp);
    if i==7 || (isillumination~=1 && (i==16||i==17))
        title("unrecognized face"); % mached no
    else    
        title(strcat("RNo.",string(myindexes(i)))); % mached no
    end
    hold on;
    end
end
...
TestDataPatternVectors=TPVs'; % 39x20
